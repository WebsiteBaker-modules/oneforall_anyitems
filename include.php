...
if (!function_exists('oneforall_anyitems')) {
	function oneforall_anyitems($section_id, $num_of_items = 3, $order = true) {
		...
		$setting_header = '<div id="mod_oneforall_anyitems_wrapper">
		<h2>OneForAll AnyItems</h2>';

		$setting_item_loop = '<a href="[LINK]">
		<div class="mod_oneforall_anyitems_item">
		<h3>[TITLE]</h3>
		<ul>
			<li class="mod_oneforall_anyitems_field_1">[FIELD_1]</li>
			<li class="mod_oneforall_anyitems_field_2">[FIELD_2]</li>
			<li class="mod_oneforall_anyitems_field_3">[FIELD_3]</li>
		</ul>
		</div>
		</a>
		';
		...
		if (empty($mod_name)) {
			...
			// Get position ordering
			$order = $order == true ? 'ASC' : 'DESC';
			// Query items
			$query_items = $database->query("SELECT * FROM ".TABLE_PREFIX."mod_".$mod_name."_items WHERE section_id = '".$section_id."' AND active = '1' AND title != '' ORDER BY position ".$order.$limit);
